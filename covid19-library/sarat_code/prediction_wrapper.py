...
import pandas as pd
import requests
from models.seirs import SEIRSModel
...
def lambda_handler(event, context):
    ...
    try:
        ...
        s3_client.put_object(Bucket=COVID_S3_BUCKET, Key=OUTPUT_JSON_KEY, Body=(bytes(json.dumps(json_data).encode('UTF-8'))))
...
def _predict_for_region(region_name,region_type,model_name,model_params,metadata,observations):
    ...
    #fetch model params
    region_params = _get_model_params_for_region(region_type,region_name,model_name,model_params)
    ...
    output_df = switcher.get(model_name,_invoke_unknown)(region_name,region_type,region_params,region_metadata,region_observations)
...
def _get_model_params_for_region(region_type,region_name,model_name,model_params):
    for params in model_params["regional_model_params"]:
        if params["region_type"] == region_type and params["region_name"] == region_name and params["model_name"] == model_name:
